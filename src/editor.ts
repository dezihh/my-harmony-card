...
class MYHarmonyCardEditor extends LitElement {
  ...
  setDimensions(dimensions) {
    ...
    return html`
      ...
      <br>
      <label for="scale" style="margin-top:0px;">Card Scale: ${actualDimensions.scale}</label><br>
      <input type="range" min="0.5" max="1.5" step="0.01" .value="${actualDimensions.scale}" id="scale" name="scale" @input=${this.dimensionsConfigChanged} style="width: 40ch;">
      <br><br>
        ...
        <div class="outer-container">
          ${activityKeys.map((key, index) => {
            ...
            if (!activityData.device_id) {
              activityData.device_id = '-1';
              const updatedConfig = { ...this._config.activities, [key]: activityData };
            ...
            return html`
              <div class="activity-container">
                <div class="form-group">
                  <label for="activity-${index}-key" class="activity-identifier">${key}:</label>
                  <input type="text" id="activity-${index}-device-id" .value="${activityData.device_id}"
  ...
  _updateDeviceId(key, value) {
    const updatedActivity = { ...this._config.activities[key], device_id: value };
    const updatedActivities = { ...this._config.activities, [key]: updatedActivity };
